# WorkSync MVP Epics & User Stories (Final)

This file contains the finalized epics and user stories for the WorkSync MVP, based on the approved architecture and resolved clarifications.
...
- Story 1.2: Tenant User Management (UI)
  - As a tenant Admin, I want to create and manage tenant users in the application UI so that I can assign team members and viewers.
  - Acceptance: Admin UI allows create/edit users, assign tenant membership, and set `TenantUser.role`.
...
- Story 1.3: Tenant User Role Editing
  ...
  - Acceptance: Admin UI allows changing roles between Admin, Project Manager, Team Member, and Viewer.

- Story 1.4: Superuser Tenant Administration (Django admin)
...
- Story 5.1: Create Weekly Resource Allocation
  - As an Admin or Project Manager, I want to create weekly `ResourceAllocation` records so that workload planning is explicit.
  - Acceptance: allocations store user, week (start date), allocated hours, and optional notes; allocations are independent of tasks.

- Story 5.2: Compute Utilization
  - As an Admin or Project Manager, I want utilization calculated from allocated hours and a fixed 40-hour week so that I can see workload capacity.
  - Acceptance: system computes utilization = allocated hours / 40 and flags >100% as over-allocated.
...
- Story 5.4: View Personal Utilization
  ...
  - Acceptance: personal workload view shows allocated hours, utilization percentage, and over-allocation warnings.
...
- Story 7.2: Minimal Audit Logging
  ...
  - Acceptance: login events and project/task create/update/delete/status changes are recorded in backend logs.
...
## Finalization Notes
- Tenant-level Admin user management is implemented in the application UI for MVP; Django admin remains available to superusers/developers.
- `ResourceAllocation` records are independent planned allocations by user/week and are not required to link to tasks for MVP.
- Audit logs are backend-only for MVP; no UI audit viewer is necessary.
